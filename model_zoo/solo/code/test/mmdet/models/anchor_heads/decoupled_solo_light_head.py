...
def points_nms(heat, kernel=2):
    ...
    #keep=(torch.abs(hmax[:, :, :-1, :-1] -  heat) < 0.00001).float()
    return heat * keep
...
class DecoupledSOLOLightHead(nn.Module):
    def __init__(self,
        ...
        self._init_layers()
        self.coord_feature = self.init_coord()
    ...
    def init_coord(self):
        #x_value = [96,96,48,24,24]
        #y_value = [64,64,32,16,16]
        x_value = [80,80,40,20,20]
        y_value = [80,80,40,20,20]
        coord_fea = []
        for i in range(len(x_value)):
            x_range = torch.linspace(-1, 1, x_value[i])
            y_range = torch.linspace(-1, 1, y_value[i])
            y, x = torch.meshgrid(y_range, x_range)
            y = y.expand([1, 1, -1, -1])
            x = x.expand([1, 1, -1, -1])
        
            coord_feat = torch.cat([x, y], 1)
            coord_fea.append(coord_feat.cuda())
            #coord_fea.append(coord_feat)
        return coord_fea

    def forward(self, feats, mode, eval=False,):
        new_feats = self.split_feats(feats)
        featmap_sizes = [featmap.size()[-2:] for featmap in new_feats]
        upsampled_size = (featmap_sizes[0][0] * 2, featmap_sizes[0][1] * 2)
        if mode=='float':
            coord_feature = self.coord_feature
            ins_pred_x, ins_pred_y, cate_pred = multi_apply(self.forward_single, new_feats, tuple(coord_feature),
                                                            list(range(len(self.seg_num_grids))),
                                                            eval=eval, upsampled_size=upsampled_size)
        #if mode ~= 'quant':
        else:
            coord_feature = self.coord_feature
            ins_pred_x, ins_pred_y, cate_pred = multi_apply(self.forward_single_quant, new_feats, tuple(coord_feature),
                                                            list(range(len(self.seg_num_grids))),
                                                            eval=eval, upsampled_size=upsampled_size)
        return ins_pred_x, ins_pred_y, cate_pred

    def split_feats(self, feats):
        return (F.interpolate(feats[0], scale_factor=0.5, mode='nearest'), 
                feats[1], 
               feats[2], 
                feats[3], 
                F.interpolate(feats[4], size=feats[3].shape[-2:], mode='nearest'))

    def forward_single_quant(self, x, coord_feat, idx, eval=False, upsampled_size=None):
        ins_feat = x
        cate_feat = x
        coord_feat = self.quant(coord_feat)
        ins_feat = torch.cat([ins_feat, coord_feat], 1)

        for ins_layer in self.ins_convs:
            ins_feat = ins_layer(ins_feat)

        #ins_feat = self.upsample(ins_feat)
        ins_feat = F.interpolate(ins_feat, scale_factor=2, mode='nearest')
        ...
        for i, cate_layer in enumerate(self.cate_convs):
            if i == self.cate_down_pos:
                ...
                cate_feat = F.interpolate(cate_feat, size=seg_num_grid, mode='nearest')
            cate_feat = cate_layer(cate_feat)

        cate_pred = self.dsolo_cate(cate_feat)
        if eval:
            ins_pred_x = F.hardsigmoid(ins_pred_x)
            ins_pred_y = F.hardsigmoid(ins_pred_y)
            if (ins_pred_x.shape[-2] != upsampled_size[1] and ins_pred_x.shape[-1] != upsampled_size[0]):
                ins_pred_x = F.interpolate(ins_pred_x, size=upsampled_size, mode='nearest')
                ins_pred_y = F.interpolate(ins_pred_y, size=upsampled_size, mode='nearest')
            heat = F.hardsigmoid(cate_pred)
            hmax = nn.functional.max_pool2d(heat, (2, 2), stride=1, padding=1)
            ins_pred_x = self.dequant(ins_pred_x)
            ins_pred_y = self.dequant(ins_pred_y)
            hmax = self.dequant(hmax)
            heat = self.dequant(heat)
            keep = (hmax[:, :, :-1, :-1] == heat).float()
            cate_preds = (heat * keep).permute(0,2,3,1)
        return ins_pred_x, ins_pred_y, cate_preds

    def forward_single(self, x, coord_feat, idx, eval=False, upsampled_size=None):
        ins_feat = x
        cate_feat = x
        # ins branch
        # concat coord
        ...
        #ins_feat = self.quant(ins_feat)
        #cate_feat = self.quant(cate_feat)
        #x = self.quant(x)
        ins_feat = torch.cat([ins_feat, coord_feat], 1)
        ...
        if eval:
            ...
            ins_pred_y = F.hardsigmoid(ins_pred_y)
            if (ins_pred_x.shape[-2] != upsampled_size[1] and ins_pred_x.shape[-1] != upsampled_size[0]):
                #ins_pred_x = F.interpolate(ins_pred_x, size=upsampled_size, mode='bilinear')
                #ins_pred_y = F.interpolate(ins_pred_y, size=upsampled_size, mode='bilinear')
                ins_pred_x = F.interpolate(ins_pred_x, size=upsampled_size, mode='nearest')
                ins_pred_y = F.interpolate(ins_pred_y, size=upsampled_size, mode='nearest')
            heat = F.hardsigmoid(cate_pred)
            hmax = nn.functional.max_pool2d(heat, (2, 2), stride=1, padding=1)
