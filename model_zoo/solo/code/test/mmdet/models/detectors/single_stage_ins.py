...
#        return seg_result  
    ...
    def simple_test(self, img, mode):
        x = self.extract_feat(img)
        outs = self.bbox_head(x, mode, eval=True)

        return outs  
    def simple_test_quant(self, img, mode):
        img = self.quant(img)
        x = self.extract_feat(img)
        outs = self.bbox_head(x, mode, eval=True)
    ...
    def postprocess_api(self, outs, img_meta, rescale=False):
    ...
    def forward(self, img):
        ...
        """
        if self.mode=='float':
            return self.simple_test(img, self.mode)
        else:
            return self.simple_test_quant(img, self.mode)
