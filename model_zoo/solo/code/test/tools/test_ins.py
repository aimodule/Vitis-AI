...
def single_gpu_test(model, data_loader, mode, show=False, verbose=True):
    model.eval()
    results = []
    input = torch.randn([1,3,640,640]).cpu()
    #macs, params = profile(model.cpu(), inputs=(input, ))
    #print("macs",macs)
    #print("param",params)
    #get_model_mac(model)
    if mode == 'quant':
       model.module.mode = 'quant'
       quantizer = torch_quantizer('calib',model.module, input,device=torch.device("cuda"), bitwidth = 8)
       quant_model = quantizer.quant_model
    if mode == 'deploy' or mode == 'test':
       model.module.mode = 'quant'
       quantizer = torch_quantizer('test',model.module, input,device=torch.device("cuda"), bitwidth = 8)
       quant_model = quantizer.quant_model
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            if mode =='float':
                ...
                outs = model(data['img'][0].cuda())
                seg_result = model.module.postprocess_api(outs, data['img_meta'][0].data[0], rescale= False,)

            else:
                if mode == 'quant':
                    outs = quant_model(data['img'][0].cuda())
                    new_outs = []
                    in_len = 5 
                    for j in range(len(outs)//in_len):
                        new_outs.append(outs[j*in_len:(j+1)*in_len])
                    new_outs = tuple(new_outs)
                    seg_result = model.module.postprocess_api(new_outs, data['img_meta'][0].data[0], rescale= False,)
                    if i == 100:
                ...
                if mode == 'deploy':
                    ...
                    seg_result = model.module.postprocess_api(new_outs, data['img_meta'][0].data[0], rescale= False,)
                    if i == 1:
                        quantizer.export_xmodel('quantize_result/',deploy_check=True)
                        exit()
...
def parse_args():
    ...
    parser.add_argument(
        '--mode',
        help='definition differents run mode ',
        type=str)
...
def main():
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(model, data_loader, mode)
    else:
